...
DATDIR = os.path.join(ROOTDIR, 'Project1\data')
...
COLWIDTHS = {'Volume': 14, 'Date': 11, 'Adj_close': 19, 'Close': 10, 'Open': 6, 'High': 20}
# ----------------------------------------------------------------------------
...
def get_tics(pth):
    ...
    pth = os.path.join(ROOTDIR, 'TICKERS.txt')
    tickers = {}
    with open(pth) as stock:
...
def read_dat(tic):
    ...
    stock_info = os.path.join(DATDIR, f'{tic}_prc.dat')
    with open(stock_info) as si:
        lines = [line.strip() for line in si]
    return lines
# ----------------------------------------------------------------------------
#   Please complete the body of this function, so it matches its docstring
#   description. See the assessment description file for more information.
...
def line_to_dict(line):
    ...
    - For each line in the file, extract the correct value for each column
      sequentially.
    """
    col_width = {col: COLWIDTHS[col] for col in COLUMNS}
    result = {}
    start = 0
    for col in COLUMNS:
        width = col_width[col]
        number = line[start: start + width].strip()
        result[col] = number
        start = start + width
    return result
# ----------------------------------------------------------------------------
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    """
    tic_exchange_dic = get_tics('TICKERS.txt')

    if tickers_lst is None:
        pass

    if tickers_lst is not type(list):
        raise TypeError("Sorry only list type is allowed")

    if tickers_lst is not None and len(tickers_lst) == 0:
        raise Exception("Sorry, Can't be empty list")

    for ticker in tickers_lst:
        if ticker.lower() not in tic_exchange_dic:
            raise Exception("sorry, ticker not found in Tic_exchange_dic")
# ----------------------------------------------------------------------------
#   Please complete the body of this function, so it matches its docstring
#   description. See the assessment description file for more information.
# ----------------------------------------------------------------------------
